...
function driver(x){
	...
	for(let i = 0; i < sigmaM.length; i++){
		...
		if(sigma_m % 2 == 0){
			...
			while(t <= x){
				//console.log("s("+(Math.pow(2, j)*m)+") = " + t)
				characFunc[(t/2)-1]++
				t = 2 * t + sigma_m
				//j++
			}
		...
		//s(m^2) is even
			...
			if(s_mSq <= x){
				characFunc[(s_mSq/2)-1]++
			}
		}
	}
	recUntouch(characFunc)
	return 
}
...
function recUntouch(characFunc){
	let accPre = [0,0,0,0,0,0,0,0]
	accPre[0]++
	for(let i = 0; i < characFunc.length; i++){
		accPre[characFunc[i]]++
		//console.log("characFunc ["+i+"] = " + characFunc[i])
	}
	
	for(let j = 0; j < 8; j++){
		console.log(j + " parent count "+ accPre[j] )
	}
